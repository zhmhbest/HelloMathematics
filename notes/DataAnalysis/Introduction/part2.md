...
- 差商的值与节点的排列次序无关
- <span class='mark'>若$f(x)$为$n$次多项式，则$f[x_0,x_1,\cdots,x_n]$为一个常数，$f[x_0,x_1,\cdots,x_n,x_{n+1}] = 0$。</span>
- $f[x_0, \cdots, x_k] = \sum\limits_{j=0}^{k} \dfrac{f(x_j)}{(x_j-x_0)\cdots(x_j-x_{j-1})(x_j-x_{j+1})(x_j-x_k)}$
- <span class='mark'>$f[x_0,\cdots,x_k] = \dfrac{f^{(k)}(ξ)}{k!}, \,\,\, ξ∈[a,b]$</span>
...
对于多项式$P_m(x)$、$P_n(x)$和权函数$ρ(x)≥0$，定义内积

$$\left<P_m(x),P_n(x)\right> =  \int_{a}^{b} P_m(x)P_n(x)ρ(x) \,{\rm d}x$$

若$\left<P_m(x),P_n(x)\right> = 0 { \ \ \ } m≠n$，则这些多项式则称为**正交多项式（Orthogonal Polynomials）**，若此外还满足$\left<P_n(x),P_n(x)\right> = 1$，则称为**规范正交多项式**。
