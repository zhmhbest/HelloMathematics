...
特别有当$k=0$时，$\sum\limits_{i=0}^{n} l_i(x) = 1$
...
函数$f(x)$满足$f(-1)=-2$，$f(0)=-1$、$f(1)=0$、$f'(0)=0$，求不超过3次的满足$p'_3(0) = f'(0)$的多项式$p_3(x)$。
...
\begin{array}{l}
        = N_2(x) + λω_{3}(x)
\\\\    = f(x_0) + f[x_0,x_1](x-x_0) + f[x_0,x_1,x_2](x-x_1) + λ(x-x_0)(x-x_1)(x-x_2)
\end{array}$$
