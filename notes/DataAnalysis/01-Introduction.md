...
- **模型误差**：无法避免，数据分析不研究。
- **观测误差**：无法避免，数据分析不研究。
- **截断误差（又称方法误差）**：人为主观采用某种近似方法造成。
- **舍入误差（又称计算误差）**：计算机只能存放浮点数近似值造成。
...
>设$x$为$x^*$的近似值，如果$x$的**绝对误差限**$e$是它的某一数位的半个单位，并且从$x$左起第一个非零数字到该数位共有$n$位，则称这$n$个数字为$x$的有效数字，即$x$近似$x^*$时有$n$位有效数字。
>此外，我们认为精确值的有效数字有无穷多位。
...
当提供误差范围时，可利用以下公式求有效数字。

$$|e| ≤ \frac{1}{2}×10^{m-n} ≤ bound(e)$$
...
- 令$|\sqrt{2} - x| ≤ \frac{1}{2}×10^{1-n} ≤ \frac{1}{2}×10^{-5}$
- 得$n=6$，$x=1.41421$。
